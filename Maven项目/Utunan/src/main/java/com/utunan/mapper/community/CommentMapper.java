...
public interface CommentMapper {
    ...
    /*根据quizId返回评论列表（王碧云）*/
    List<Answer> findCommentListByQuizId(@Param("quizId") Long quizId);

    /*根据commentId返回子评论列表（王碧云）*/
    List<Answer> findChildCommentListByCommentId(@Param("commentId") Long commentId);

    /*王碧云 根据热度返回评论列表*/
    List<Answer> findCommentListByPraiseCount(@Param("quizId") Long quizId);

    /*王碧云 根据quizId获取父级为null的评论数*/
    Long countCommentByQuizId(@Param("quizId") Long quizId);

    //根据搜索条件返回评论列表
    List<Answer> findCommentListBySearch(@Param("searchValue") String searchValue);
